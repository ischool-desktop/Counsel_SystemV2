...
{
    public partial class StudHome_VisitFilterForm : FISCA.Presentation.Controls.BaseForm
    {
        private List<string> _StudentIDList = new List<string>();

        public StudHome_VisitFilterForm(List<string> studentIDList)
        {
        ...
        private void buttonX1_Click(object sender, EventArgs e)
        {
            ...
            var showContactDetial = checkBox6.Checked;
            ...
            printWorker.DoWork += delegate
            {
                ...
                var HomeVisitRecordList = _AccessHelper.Select<DAO.UDT_Counsel_home_visit_RecordDef>("ref_student_id in (" + string.Join(",", _StudentIDList) + ")");
                ...
                foreach (var homeVisitRecord in HomeVisitRecordList)
                {
                    ...
                    {
                        if (homeVisitRecord.schoolyear != filterSchoolyear || homeVisitRecord.semester != filterSemester)
                        {
                            continue;
                        }
                    }
                    ...
                    {
                        if (homeVisitRecord.authorRole != filterAuthorRole)
                        {
                            continue;
                        }
                    }
                    ...
                    if (!dicHomeVisitRecord.ContainsKey("" + homeVisitRecord.StudentID))
                    {
                        dicHomeVisitRecord.Add("" + homeVisitRecord.StudentID, new List<DAO.UDT_Counsel_home_visit_RecordDef>());

                        dicHomeVisitRecord["" + homeVisitRecord.StudentID].Add(homeVisitRecord);
                    ...
                    printWorker.ReportProgress(20 + 50 * progressCount / HomeVisitRecordList.Count);
                }
                #endregion
                ...
                var contactTitleList = new List<string>();
               ...
               basicDataTitleList.Add("聯繫編號");
               basicDataTitleList.Add("學年度");
               basicDataTitleList.Add("學期");
               basicDataTitleList.Add("聯繫日期");
               basicDataTitleList.Add("聯繫時間");
               basicDataTitleList.Add("聯繫事由");
               basicDataTitleList.Add("聯繫方式");

               //基本資料Title--多項目
               basicDataTitleList.Add("參與成員");
               basicDataTitleList.Add("聯繫成員");
               basicDataTitleList.Add("聯繫類別");
               ...
               basicDataTitleList.Add("記錄人");
                ...
                foreach (var student_id in _StudentIDList)
                {
                    ...
                    if (dicHomeVisitRecord.ContainsKey(student_id))
                    {
                        foreach (var HomeVisitRecord in dicHomeVisitRecord[student_id])
                        {
                            //每有一筆晤談紀錄 就有一個新Row
                            ...
                            //聯繫編號
                            row["聯繫編號"] = HomeVisitRecord.home_visit_no;
                            //學年度
                            row["學年度"] = HomeVisitRecord.schoolyear;
                            //學期
                            row["學期"] = HomeVisitRecord.semester;
                            //聯繫日期
                            row["聯繫日期"] = HomeVisitRecord.home_visit_date.Value.ToShortDateString();
                            //聯繫時間
                            row["聯繫時間"] = HomeVisitRecord.home_visit_time;
                            //聯繫事由
                            row["聯繫事由"] = HomeVisitRecord.Cause;
                            //聯繫方式
                            row["聯繫方式"] = HomeVisitRecord.home_visit_type;
                            //地點
                            row["地點"] = HomeVisitRecord.Place;
                            //內容要點
                            row["內容要點"] = HomeVisitRecord.ContentDigest;
                            //記錄人的登入帳號
                            row["記錄人的登入帳號"] = HomeVisitRecord.AuthorID;
                            //記錄人的姓名
                            row["記錄人的姓名"] = HomeVisitRecord.AuthorName;
                            //是否公開
                            row["是否公開"] = (HomeVisitRecord.isPublic ? "是" : "否");
                            //記錄人
                            row["記錄人"] = HomeVisitRecord.authorRole;

                            #region row["參與成員"] 整理
                            XmlDocument doc1 = new XmlDocument();
                            //幫忙加根目錄
                            string xmlContent1 = "<root>" + HomeVisitRecord.Attendees + "</root>";
                            ...
                            foreach (XElement abs in xmlabs1.Elements("Item"))
                            {
                                ...
                                //假如使用者有需要參與成員分析資料，則新增欄位，並在其值填 "是"
                                if (!dt.Columns.Contains("參與成員:" + attendees))
                                {
                                    dt.Columns.Add("參與成員:" + attendees);
                                    attendeesDataTitleList.Add("參與成員:" + attendees);
                                }
                                //參與成員
                                row["參與成員"] = attendees_for_basic;
                                row["參與成員:" + attendees] = "是";
                            ...
                            //幫忙加根目錄
                            string xmlContent2 = "<root>" + HomeVisitRecord.contact + "</root>";
                            ...
                            string Contact = "";
                            string Contact_for_basic = "";

                            foreach (XElement abs in xmlabs2.Elements("Item"))
                            {

                                Contact_for_basic += abs.Attribute("name").Value;
                                if (abs.Attribute("name").Value == "其他")
                                {
                                    if (abs.Attribute("remark") != null)
                                        Contact_for_basic += ":" + abs.Attribute("remark").Value;
                                }
                                ...
                                Contact = abs.Attribute("name").Value;
                                if (abs.Attribute("name").Value == "其他")
                                {
                                    if (abs.Attribute("remark") != null)
                                        Contact += ":" + abs.Attribute("remark").Value;
                                }
                                //假如使用者有需要聯繫成員分析資料，則新增欄位，並在其值填 "是"
                                if (!dt.Columns.Contains("聯繫成員:" + Contact))
                                {
                                    contactTitleList.Add("聯繫成員:" + Contact);
                                    dt.Columns.Add("聯繫成員:" + Contact);
                                }



                                //聯繫成員
                                row["聯繫成員"] = Contact_for_basic;
                                row["聯繫成員:" + Contact] = "是";
                            }
                            #endregion
                            ...
                            foreach (XElement abs in xmlabs3.Elements("Item"))
                            {
                                ...
                                //案件類別
                                row["聯繫類別"] = CounselTypeKind_for_basic;
                                row["聯繫類別:" + CounselTypeKind] = "是";
                ...
                wb.Open(new MemoryStream(Properties.Resources.家庭聯繫紀錄篩選), FileFormatType.Excel2003);
                ...
                //聯繫成員分析資料
                if (showContactDetial)
                {
                    foreach (string title in contactTitleList)
                    {
                        cs[0, col_index].PutValue(title);
            ...
            {
                FISCA.Presentation.MotherForm.SetStatusBarMessage("匯出家庭聯繫類別", e2.ProgressPercentage);
            };
